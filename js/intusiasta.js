/* ══════════════════════════════════════════
   ...
   (function carregarFotoPerfil() {
       if (!cpfLogado) return;
       fetch(`../php/buscar_foto.php?cpf=${encodeURIComponent(cpfLogado)}`)
           .then(r => r.json())
           .then(d => { if (d.foto_perfil) avatarEl.src = d.foto_perfil; })
           .catch(() => {});
   })();
   ...
   inputFotoP.addEventListener("change", () => {
       ...
       fetch("../php/upload_foto.php", { method: "POST", body: fd })
           .then(r => r.json())
           .then(d => {
               if (d.caminho) { avatarEl.src = d.caminho; showToast("Foto de perfil atualizada! ✅"); }
               else showToast(d.erro || "Erro ao salvar foto.");
           })
   ...
   function handleFiles(files, type) {
       Array.from(files).forEach(file => {
           ...
           rm.textContent = "✕";
           rm.onclick = () => { attachedFiles.splice(attachedFiles.indexOf(file), 1); item.remove(); };
           item.appendChild(rm);
   ...
   /* ══════════════════════════════════════════
      PUBLICAR POST → BANCO (FormData com mídias)
      ══════════════════════════════════════════ */
   document.getElementById("submitPost").addEventListener("click", () => {
       const texto = document.getElementById("postText").value.trim();
   
       if (!texto && attachedFiles.length === 0) { showToast("Escreva algo antes de publicar!"); return; }
       if (!cpfLogado) { showToast("Você precisa estar logado para publicar!"); return; }
   
       const fd = new FormData();
       fd.append("cpf",      cpfLogado);
       fd.append("conteudo", texto);
       attachedFiles.forEach(f => fd.append("midias[]", f));
   
       fetch("../php/salvar_post.php", { method: "POST", body: fd })
           .then(r => r.json())
           .then(d => {
               if (d.erro) { showToast(d.erro); return; }
   
               /* Monta mídia com URLs permanentes vindas do servidor */
               let mediaHTML = "";
               (d.midias || []).forEach(m => {
                   mediaHTML += m.tipo === "video"
                       ? `<div class="post-media"><video controls style="width:100%;max-height:340px;"><source src="${m.url}"></video></div>`
                       : `<div class="post-media"><img src="${m.url}" alt="imagem do post"></div>`;
               });
   
               criarCardPost({
                   id: d.post_id, nome: nomeLogado, cpf: cpfLogado,
                   foto_perfil: avatarEl.src, conteudo: texto,
                   data_postagem: "Agora mesmo", mediaHTML,
                   total_curtidas: 0, total_comentarios: 0,
                   curtiu: false, comentarios: [], dono: true
               }, true);
   ...
   /* ══════════════════════════════════════════
      CARREGAR POSTS DO BANCO AO ABRIR A PÁGINA
      ══════════════════════════════════════════ */
   (function carregarPosts() {
       const url = `../php/buscar_posts.php${cpfLogado ? "?cpf=" + encodeURIComponent(cpfLogado) : ""}`;
       fetch(url)
           .then(r => r.json())
           .then(posts => {
               posts.forEach(p => {
                   ...
                   (p.midias || []).forEach(m => {
                       mediaHTML += m.tipo === "video"
                           ? `<div class="post-media"><video controls style="width:100%;max-height:340px;"><source src="${m.url}"></video></div>`
                           : `<div class="post-media"><img src="${m.url}" alt=""></div>`;
                   });
                   criarCardPost({
                       ...
                       mediaHTML,
                       total_curtidas:    parseInt(p.total_curtidas),
                       total_comentarios: parseInt(p.total_comentarios),
                       curtiu:      p.curtiu,
   ...
   function criarCardPost(p, noTopo) {
       ...
       const avatarHTML = p.foto_perfil && !p.foto_perfil.endsWith("user.png")
       ...
       const likedFill  = p.curtiu ? `fill="#e74c3c" stroke="#e74c3c"` : `fill="none" stroke="currentColor"`;
       const likedLabel = p.curtiu ? "Curtido" : "Curtir";
       const likedClass = p.curtiu ? "action-btn liked" : "action-btn";
   
       /* Comentários já salvos */
       let comentariosHTML = "";
       (p.comentarios || []).forEach(c => {
           const fotoC = c.foto_perfil && !c.foto_perfil.endsWith("user.png")
               ? `<img src="${c.foto_perfil}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
               : "👤";
           comentariosHTML += `
               <div class="comment">
                   <div class="avatar" style="width:34px;height:34px;font-size:0.9rem;">${fotoC}</div>
                   <div class="comment-bubble">
                       <strong>${escapeHTML(c.nome)}</strong>${escapeHTML(c.comentario)}
                   </div>
               </div>`;
       });
   
       const curtLabel = `${p.total_curtidas} curtida${p.total_curtidas !== 1 ? "s" : ""}`;
       const comsLabel = `${p.total_comentarios} comentário${p.total_comentarios !== 1 ? "s" : ""}`;
       ...
       card.innerHTML = `
           ...
           <div class="post-stats">
               <span class="reaction-icons">${p.total_curtidas > 0 ? "❤️" : ""}</span>
               <span class="stats-texto">${curtLabel} · ${comsLabel}</span>
           </div>
           <div class="post-actions">
               <button class="${likedClass}" onclick="toggleLike(this)">
                   <svg viewBox="0 0 24 24" ${likedFill} stroke-width="2"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>
                   ${likedLabel}
               </button>
           ...
           <div class="comments-section">
               ...
               <div class="comment-input-row">
                   <div class="avatar" style="width:34px;height:34px;font-size:0.9rem;">${
                       avatarEl.src && !avatarEl.src.endsWith("user.png")
       ...
       noTopo ? feed.insertBefore(card, feed.firstChild) : feed.appendChild(card);
   }
   ...
   window.toggleLike = function(btn) {
       if (!cpfLogado) { showToast("Faça o login para curtir!"); return; }
       ...
       .then(d => {
           ...
           const curtiu = d.acao === "curtido";
           btn.classList.toggle("liked", curtiu);
           btn.innerHTML = curtiu
           ...
           /* Atualiza contador */
           const statsEl = card.querySelector(".stats-texto");
           const matchC  = statsEl.textContent.match(/·\s*(\d+)/);
           const totalC  = matchC ? parseInt(matchC[1]) : 0;
           card.querySelector(".reaction-icons").textContent = curtiu ? "❤️" : "";
           statsEl.textContent = `${d.total} curtida${d.total !== 1 ? "s" : ""} · ${totalC} comentário${totalC !== 1 ? "s" : ""}`;
       })
       .catch(() => showToast("Erro ao registrar curtida."));
   };
   ...
   window.submitComment = function(btn) {
       ...
       if (!texto) return;
       if (!cpfLogado) { showToast("Faça o login para comentar!"); return; }
       ...
       fetch("../php/salvar_comentario.php", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ post_id: postId, cpf: cpfLogado, comentario: texto })
       })
       .then(r => r.json())
       .then(d => {
           if (d.erro) { showToast(d.erro); return; }
           ...
           const fotoComent = avatarEl.src && !avatarEl.src.endsWith("user.png")
               ? `<img src="${avatarEl.src}" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
               : "👤";
           ...
           comment.innerHTML = `
               <div class="avatar" style="width:34px;height:34px;font-size:0.9rem;">${fotoComent}</div>
               <div class="comment-bubble">
           ...
           /* Atualiza contador */
           const statsEl = card.querySelector(".stats-texto");
           const matchL  = statsEl.textContent.match(/^(\d+)/);
           const totalL  = matchL ? parseInt(matchL[1]) : 0;
           const matchC  = statsEl.textContent.match(/·\s*(\d+)/);
           const totalC  = matchC ? parseInt(matchC[1]) + 1 : 1;
           statsEl.textContent = `${totalL} curtida${totalL !== 1 ? "s" : ""} · ${totalC} comentário${totalC !== 1 ? "s" : ""}`;
       })
       .catch(() => showToast("Erro ao salvar comentário."));
   };
   ...
   window.editarPost = function(btn) {
       ...
       textarea.style.cssText = "width:100%;min-height:80px;padding:10px 18px;border:1.5px solid aquamarine;border-radius:8px;font-size:0.9rem;font-family:Arial,sans-serif;resize:vertical;outline:none;margin-bottom:8px;box-sizing:border-box;";
       ...
       acoes.innerHTML = `
           <button class="btn-cancelar" onclick="cancelarEdicao(this,'${escapeAttr(textoAtual)}')">Cancelar</button>
           <button class="btn-publicar" onclick="salvarEdicao(this,'${postId}')">Salvar</button>`;
       textarea.insertAdjacentElement("afterend", acoes);
       textarea.focus();
   };
   
   window.cancelarEdicao = function(btn, textoOriginal) {
       const card     = btn.closest(".post-card");
       const textarea = card.querySelector(".edit-area");
       const textoDiv = document.createElement("div");
       textoDiv.className        = "post-text";
       textoDiv.dataset.original = textoOriginal;
       textoDiv.innerHTML        = escapeHTML(textoOriginal).replace(/\n/g,"<br>");
   ...
   window.salvarEdicao = function(btn, postId) {
       ...
       fetch("../php/editar_post.php", {
           method: "POST",
           headers: { "Content-Type": "application/json" },
           body: JSON.stringify({ post_id: parseInt(postId), cpf: cpfLogado, conteudo: novoTexto })
       })
       .then(r => r.json())
       .then(d => {
           if (d.erro) { showToast(d.erro); return; }
   ...
   window.toggleFollow = function(btn) {
       if (btn.classList.contains("seguindo")) {
           btn.classList.remove("seguindo"); btn.textContent = "+ Seguir";
       } else {
           btn.classList.add("seguindo"); btn.textContent = "✓ Seguindo";
